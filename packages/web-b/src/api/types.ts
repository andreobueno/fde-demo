...
export interface Analyst { id: string; name: string; role: AnalystRole }
...
export interface RiskSignal {
  id: string; caseId: string; code: string; title: string; description: string;
  severity: SignalSeverity; weight: number;
}
export interface KycCase {
  id: string; reference: string; customerId: string; status: CaseStatus; riskLevel: RiskLevel;
  riskScore: number; assignedTo: string | null; createdAt: string; updatedAt: string;
...
export type CaseDetail = KycCase & { customer: Customer; signals: RiskSignal[]; audit: AuditEvent[]; allowedActions: CaseAction[] };
export interface CaseListResponse { items: KycCase[]; total: number; page: number; pageSize: number }
export interface CaseStats { byStatus: Record<CaseStatus, number>; byRiskLevel: Record<RiskLevel, number>; total: number }
export interface ApiError { error: { code: string; message: string; details?: unknown } }
export interface ActionResponse extends KycCase { audit: AuditEvent[]; allowedActions: CaseAction[] }
