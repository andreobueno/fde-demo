...
import { DEFAULT_RISK_POLICY, POLICY_RULE_CODES } from '../domain/riskPolicy.js';
...
interface PolicyChangeRow {
  ...
  actor_name: string;
  changes: string;
  recomputed_cases: number;
  created_at: string;
}

function rowToPolicyChange(r: PolicyChangeRow): RiskPolicyChange {
  return {
    id: r.id,
    version: r.version,
    actorId: r.actor_id,
...
export function listPolicyChanges(db: Db): RiskPolicyChange[] {
  return db
    .prepare<[], PolicyChangeRow>('SELECT * FROM risk_policy_changes ORDER BY version DESC')
    .all()
...
export function latestPolicyChange(db: Db): RiskPolicyChange | null {
  const row = db
    .prepare<[], PolicyChangeRow>('SELECT * FROM risk_policy_changes ORDER BY version DESC LIMIT 1')
    .get();
