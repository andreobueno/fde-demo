...
| --- | --- | --- |
| Authentication / identity | Explicit known `x-analyst-id` required on all sensitive reads/writes, selected in the demo UI. No login or session; callers can impersonate any seeded ID. | SSO/OIDC (e.g. Entra ID, Okta) with short-lived sessions or tokens validated server-side; identity provisioned from the IdP/HR directory, not seeded; MFA per company policy. |
...
| Rate limiting / CSRF / CSP | Basic security headers, no-store API responses and a localhost-only CORS allow-list. No rate limiting; demo identity is a caller-supplied header. | Rate limiting and abuse detection at the edge, CSRF protection for cookie-based auth, strict CSP, HSTS, request body limits tuned, WAF, dependency and container scanning. |
...
- **Authentication and access administration.** Entra ID sign-in and access administration are platform features. Here RBAC is enforced, but identity remains a demo header, without production provisioning or row-level scoping.
